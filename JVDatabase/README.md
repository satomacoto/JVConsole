...
JVDatabaseは、JV-Link仕様書に準拠した差分更新機能を実装しています：

1. **lastfiletimestamp管理**
   - 各データ種別の最終ファイルタイムスタンプを自動的に記録
   - 次回更新時は、このタイムスタンプ以降のデータのみを取得
   - 重複ダウンロードを防ぎ、効率的な更新を実現

2. **データ種別ごとの独立管理**
   - RACE、DIFN等のデータ種別ごとに個別に更新状態を管理
   - 特定のデータ種別のみの更新も可能

3. **中断と再開**
   - 処理済みファイルリストを保持
   - 中断された場合でも、未処理のファイルから再開可能

### 推奨される更新頻度

- **通常データ（RACE等）**: 週1回程度（土日のレース後）
- **マスタデータ（DIFN、BLDN等）**: 月1回程度
- **リアルタイムデータ**: レース開催日当日
...
1. **大容量データのスキップ**
   ```bash
   # オッズデータ（H6、O6）は容量が大きいため、必要に応じてスキップ
   JVDatabase init --skip H6,O6
   ```

2. **期間を分割した更新**
   - JVDownloaderが自動的に月単位で分割処理
   - 大量データも効率的にダウンロード

3. **ストレージの考慮事項**
   - 1年分のフルデータ: 約10-20GB
   - Parquet形式により元データの約50%に圧縮
...
metadata.jsonのDataSpecStatusesセクションを確認してください。正常に動作していれば、各データ種別のlastfiletimestampが記録されているはずです。